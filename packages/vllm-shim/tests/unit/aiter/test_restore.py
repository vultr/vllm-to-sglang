...
def _seed(shim_home: Path, *names: str) -> Path:
    """Create the bucket directory under shim_home and drop empty CSVs."""
    bucket_dir = shim_home / "aiter" / "configs" / "gfx942-304cu"
...
def test_plan_disabled_when_no_gpu(tmp_path: Path) -> None:
    plan = plan_restore(shim_home=tmp_path, gpu=None)
...
def test_plan_disabled_when_shim_home_unresolvable() -> None:
    plan = plan_restore(shim_home=None, gpu=_GPU)
    assert plan.enabled is False
    assert plan.source is None
...
def test_plan_source_is_bucket_partitioned(tmp_path: Path) -> None:
    plan = plan_restore(shim_home=tmp_path, gpu=_GPU)
    ...
    assert plan.source == tmp_path / "aiter" / "configs" / "gfx942-304cu"
...
def test_restore_returns_empty_when_source_missing(tmp_path: Path) -> None:
    ...
    plan = plan_restore(shim_home=tmp_path / "shim-empty", gpu=_GPU)
    assert restore_configs(plan) == {}
