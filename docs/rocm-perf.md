...
The Dockerfile defends against that. After `scripts/apply-patches.sh` has replayed every patch under `patches/aiter/` onto a `patched/rocm` branch in `/sgl-workspace/aiter`, the Dockerfile runs `git -C /sgl-workspace/aiter rev-parse --short=12 HEAD` and writes the 12-char SHA to `/etc/vllm-shim/aiter-cache-key`. HEAD at that point is the tip of `patched/rocm`, so the captured SHA covers *both* the upstream pin and the local patches. `vllm_shim.cli.rocm_perf.aiter_cache_key` reads that file at launch, and the resulting `AITER_JIT_DIR` is `$VLLM_SHIM_HOME/aiter/jit/<patched_aiter_sha>`. Bumping the AITER pin rotates the SHA, the JIT dir moves to a fresh subdirectory under the same PV, and AITER recompiles. Old `jit/<old-sha>/` trees become orphaned but harmless; an operator can prune them whenever they next clean up the PV.
