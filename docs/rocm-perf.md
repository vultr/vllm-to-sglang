...
| `AITER_JIT_DIR` | `$VLLM_SHIM_HOME/aiter/jit/<aiter_sha>` | AITER's own JIT root. AITER's `aiter/jit/core.py` derives `bd_dir = $AITER_JIT_DIR/build` for HIP kernels it compiles on first call (CK GEMM/MoE kernels, fused attention paths, etc.). Default fallback is `~/.aiter/jit/build`, lost on pod restart. Anchoring under the shim's AITER root puts the cache alongside `configs/` and `shapes/` on the same PV. The `<aiter_sha>` segment is AITER's 12-char commit SHA captured at image build (or the literal `default` when absent); see "AITER JIT cache key" below. |
...
`AITER_JIT_DIR` is the one cache var the shim explicitly namespaces. AITER's JIT loader (`aiter/jit/core.py`, `_ensure_loaded`) trusts file presence and does not hash sources, so a stale `.so` on the PV will be loaded even when the AITER source bytes in the image have changed. The shim ships at least one local patch against AITER (see `docs/aiter.md`, "Upstream patches"), and bumping AITER's pinned tag (`scripts/sync-repos.sh::AITER_VERSION`) would silently run pre-bump kernel bytes if the JIT dir were not partitioned.

The Dockerfile defends against that. After applying every file under `docker/sglang/patches/`, it runs `git -C /sgl-workspace/aiter rev-parse --short=12 HEAD` and writes the 12-char AITER commit SHA to `/etc/vllm-shim/aiter-cache-key`. `vllm_shim.cli.rocm_perf._aiter_cache_key` reads that file at launch, and the resulting `AITER_JIT_DIR` is `$VLLM_SHIM_HOME/aiter/jit/<aiter_sha>`. Bumping the AITER pin rotates the SHA, the JIT dir moves to a fresh subdirectory under the same PV, and AITER recompiles. Old `jit/<old-sha>/` trees become orphaned but harmless; an operator can prune them whenever they next clean up the PV.
...
The key is deliberately the AITER SHA, *not* a hash of `docker/sglang/patches/`. Local-patch edits without an AITER bump will *not* rotate the JIT cache, so a patch change alone leaves a stale `.so` on the PV. If you edit a patch, manually clear `$VLLM_SHIM_HOME/aiter/jit/<sha>/` so AITER recompiles. Patches change rarely; AITER version bumps are the common cache-busting event, and keying on the SHA keeps the path shape clean.
