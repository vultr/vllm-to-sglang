...
**Why we need it.** `getAllAlgos` returns ~1365 unranked algorithms per shape on MI300X. Most are not viable for the specific shape being tuned, so benchmarking the full set is wasted work. Restricting to the heuristic-ranked top N cuts tune wall-clock time roughly in proportion to the cap and makes the surviving kernels likely-good rather than unsorted.
